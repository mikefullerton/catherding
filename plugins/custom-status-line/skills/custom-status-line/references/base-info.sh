...
if [ "$DAILY_AVG" -gt 14 ] 2>/dev/null; then
  RATE_7D_DISPLAY="${RATE_7D}% ${RED}[${ELAPSED_DAYS}d, ${DAILY_AVG}% ave, ${PREDICTED_7D}% projected${OVERAGE_COST}]${RST}"
else
  RATE_7D_DISPLAY="${RATE_7D}% [${ELAPSED_DAYS}d, ${DAILY_AVG}% ave, ${PREDICTED_7D}% projected${OVERAGE_COST}]"
fi
