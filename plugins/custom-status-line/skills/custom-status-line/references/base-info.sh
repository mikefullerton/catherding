...
[ "$PREDICTED_7D" -gt 100 ] && PREDICTED_7D=100

RED=$'\033[38;5;210m'
if [ "$DAILY_AVG" -gt 14 ] 2>/dev/null; then
  RATE_7D_DISPLAY="${RATE_7D}% ${RED}[${ELAPSED_DAYS}d, ${DAILY_AVG}% ave, ${PREDICTED_7D}% projected]${RST}"
else
  RATE_7D_DISPLAY="${RATE_7D}% [${ELAPSED_DAYS}d, ${DAILY_AVG}% ave, ${PREDICTED_7D}% projected]"
fi
