...
GH_CLI=$(bash "$SCRIPT_DIR/check-cli.sh" gh --version)
...
GH_AUTH=$(bash "$SCRIPT_DIR/check-auth.sh" github)

jq -n \
  --argjson cf_cli "$CF_CLI" \
  --argjson rw_cli "$RW_CLI" \
  --argjson gh_cli "$GH_CLI" \
  ...
  '{
    ...
    "github": {"cli": $gh_cli, "auth": $gh_auth}
  }'
