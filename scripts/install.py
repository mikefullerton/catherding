...
Usage:
  ...
  cc-install --from <dir>              # repoint at any directory containing *.py scripts
  cc-install --dry-run                 # just print what would change

Each `<name>.py` becomes `~/.local/bin/cc-<name>`. Useful when:
...
def main() -> int:
    ...
    ap.add_argument("--from", dest="source", default=str(DEFAULT_SOURCE),
                    help=f"Source dir containing *.py scripts (default: {DEFAULT_SOURCE})")
    ...
    scripts = sorted(p for p in src.glob("*.py") if p.is_file())
    if not scripts:
        print(f"FAIL: no *.py scripts in {src}", file=sys.stderr)
        return 1

    changed = unchanged = 0
    for script in scripts:
        name = f"cc-{script.stem}"
        target = BIN_DIR / name
