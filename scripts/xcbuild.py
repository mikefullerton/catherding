...
def main() -> int:
    ...
    # Failure path — surface the first few error lines with file:line context.
    errors = [ln for ln in out.splitlines() if "error:" in ln]
    print("FAIL: xcodebuild", file=sys.stderr)
    for ln in errors[:5]:
        print(f"  {ln}", file=sys.stderr)
    if not errors:
        # No `error:` lines — show the last 20 lines as a fallback.
        tail = out.splitlines()[-20:]
        for ln in tail:
            print(f"  {ln}", file=sys.stderr)
    if len(errors) > 5:
        print(f"  … and {len(errors) - 5} more errors (run xcodebuild directly for full log)", file=sys.stderr)
